...
def rand_samp():
    ...
    add_labels = random.sample(unlabelled_mask, 500)
...
def main():
    ...
    while active_learn_iter<50:
        ...
        model = wrn_22().to(device)
        # model = models.resnet50(pretrained=True)
        # num_ftrs = model.fc.in_features
        # model.fc = nn.Linear(num_ftrs, 10)
        model.cuda()
        if active_learn_iter != 0:
            model.load_state_dict(torch.load("cifar_resnet_rand.pt"))
        ...
        if (args.save_model):
            torch.save(model.state_dict(),"cifar_resnet_rand.pt")

        active_learn_iter = active_learn_iter + 1

    with open('results_rand_cifar.txt', 'w') as f:   
